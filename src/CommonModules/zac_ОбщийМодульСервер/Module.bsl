Процедура ЗапуститьАссистентаПоЗакупу() Экспорт
	
	Если РегистрыСведений.am_ХранилищеНастроек.ПолучитьРазрешениеНаУстановкуМинИМаксУровеньПоставки() Тогда
		ОбновитьУровеньОстаткаНоменклатуры();
	КонецЕсли;
	
	Если РегистрыСведений.am_ХранилищеНастроек.ПолучитьРазрешитьАссистентуСоздаватьЗаказыПоставщикам() Тогда 
		ОбработатьОтчётыОРознечныхПродажах();	
	КонецЕсли;
	
КонецПРоцедуры

#Область Процедуры_и_функции_для_ОбработатьОтчётыОРознечныхПродажах	

Процедура ОбработатьОтчётыОРознечныхПродажах()
	
	Организация = Константы.am_ОсновнаяОрганизация.Получить();
	...
	Пока Выборка.Следующий() Цикл
		...
		Заказать = МакУрЗапаса - ВПути - Остатки;
	...
	Для Каждого Строка Из ТЗ Цикл
		...
		Иначе //Создадим новый заказ
			...
			ЗаказПостащику.Договор 			= ПолучитьДоговорПоставки(СтруктураНовогоДокумента.Организация, Контрагент);
...
Функция ПолучитьДоговорПоставки(Организация, Поставщик)
	Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	...
	Иначе
		Договор = Справочники.ДоговорыКонтрагентов.ДоговорПоУмолчанию(Поставщик);
	КонецЕсли;
...
Функция НайдёмЗаказ(Контрагент)
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |	И ЗаказПоставщикуЗапасы.Ссылка.СостояниеЗаказа = &СостояниеЗаказа";
	Запрос.УстановитьПараметр("Контрагент", Контрагент);
	Запрос.УстановитьПараметр("СостояниеЗаказа", Справочники.СостоянияЗаказовПоставщикам.НайтиПоНаименованию("Новый"));
...
Функция ПодготовитьСтруктуруНовогоЗаказа()
	...
	СтруктураНовогоДокумента.Вставить("Организация", Организация);
...
#Область Процедуры_и_функции_для_установки_уровней_запасов	

Процедура ОбновитьУровеньОстаткаНоменклатуры()
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ МЕСЯЦ(Продажи.Период)) КАК Месяц,
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяЗапись = РегистрыСведений.УправлениеЗапасами.СоздатьМенеджерЗаписи();
		НоваяЗапись.Номенклатура 				= Выборка.Номенклатура;
		НоваяЗапись.МинимальныйУровеньЗапаса 	= 1;
		НоваяЗапись.МаксимальныйУровеньЗапаса 	= Окр((Выборка.Количество / Выборка.Месяц), 0);
